...
\hldef{y} \hlkwb{<-} \hlnum{2}\hlopt{*}\hldef{x} \hlopt{+} \hlkwd{rnorm}\hldef{(n)}
\hldef{out} \hlkwb{<-} \hlkwd{lm}\hldef{(y} \hlopt{~} \hldef{x)}
\hlkwd{library}\hldef{(knitr)}
\hlkwd{kable}\hldef{(}\hlkwd{summary}\hldef{(out)}\hlopt{$}\hldef{coef,} \hlkwc{digits}\hldef{=}\hlnum{2}\hldef{)}
\end{alltt}
\end{kframe}
...
\hline
(Intercept) & -0.07 & 0.1 & -0.68 & 0.5\\
\hline
x & 2.03 & 0.1 & 19.92 & 0.0\\
\hline
\end{tabular}
