...
% Where to put this?
%TB: After the very first paragraph, before "The benefits of early stopping..."? If so, I'd slightly rephrase it, see below
...
%TB: suggested rephrasing, if it should go after the first paragraph:
%It is worth noting that not all early stopping rules in the frequentist framework base their stopping rules on the $p$ value.  Others focus on a process of adaptation \parencite[][]{chow2008adaptive}, or on the selection of stopping criteria \parencite[e.g. estimate precision][]{KenKelly} that do not bias Type-I error rates. Yet, stopping rules are most commonly discussed with regard to the $p$ value, which is why we focus mostly on this version.

The logic of this intuition is as follows: Assume we have some sample of size $N$ on which we have run a test.  A lower p-value %TB: currently "p-value" and "$p$ value" are used interchangibly. It should definitely be uniform and I'd vote for "$p$ value" as the better version
...
For every population generated in this way, data is generated for each sampling procedure outlined below until that procedure reports data or it is certain that the procedure will not report a data set. To see the different cases, we assume that the researcher either reports (1) the full data set, (2) the mean and covariance matrix of the latent variables, and %TB: this should be an "or", shouldn't it? If not, the "either" earlier in the sentence needs to be changed
