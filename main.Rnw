...
\subsection{Validity Simulation}
To increase the accessibility of the mathematical results, we first set up a small simulation just to demonstrate the results. In this simulation, a single observed variable is investigated either with a peek preview sampling strat

\section{Longitudinal Simulation}
...
Note that Researcher 6 does at first not apply a different sampling procedure, but use the same procedure as the baseline Researcher 0. %TB: There is no condition in which researcher 0 ignores the data while there is a condition when researcher 6 ignores the data (3 or less benefitted). How is this the same sampling procedure? In my view, "only report those that benefitted" fits your explanation of simply altering the reported statistic. But adding "unless there were three or less, then he does not report" changes the sampling procedure for me
% you can view this as a two-step procedure: They first sample (fully correctly), then have the full sample on their desk. Now they kill some participants (no, not literally...) - these may be all, but it doesn't have anything to do with their sampling. The point is not very important since (following the definition) we anyway only check reported results... I changed the wording a little  
...
% TvO: You've got a point here. I just removed the sentence, the rest lives without it. 
